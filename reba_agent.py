...
from data_export import foto_analiz_export, video_analiz_export
...
with st.sidebar:
    ...
    yuz_blur = st.checkbox("Yüz anonimleştirme (KVKK)", value=True,
                           help="Yüz bölgesi bulanıklaştırılır — fotoğraf paylaşımı için zorunlu")
...
if calistir and form_tamam and yuklenen:
        ...
        try:
            ...
            if img is None:
                ...
                if not res.pose_landmarks:
                    ...
                    fs.skor = skor
                    fs.overlay_img = overlay_ciz(img.copy(), lms, skor, mode=annotation_mode, yuz_blur=yuz_blur)
    ...
    if gecerli:
        ...
        for fs in gecerli:
            ...
            if fs.skor and fs.skor.acılar:
                kamera = fs.skor.acılar.kamera_acisi
                if kamera == "lateral":
                    st.markdown(f"""
                    <div class="warn-box">
                    📐 <strong>{fs.dosya_adi}</strong>: Lateral (yan) çekim tespit edildi.
                    Üst kol ve bilek güveni %20 düşürüldü — 45° oblique açıdan çekim önerilir.
                    </div>
                    """, unsafe_allow_html=True)
                elif kamera == "frontal":
                    st.markdown(f"""
                    <div class="info-box">
                    📐 <strong>{fs.dosya_adi}</strong>: Frontal (ön) çekim tespit edildi.
                    Gövde dönüşü bu açıdan görünmeyebilir — manuel modifier kullanın.
                    </div>
                    """, unsafe_allow_html=True)
        ...
        with col_b:
            json_bytes = foto_analiz_export(form_bilgi, foto_sonuclari)
            json_ad = f"REBA_{(bolum or is_istasyonu or 'analiz').replace(' ','_')}_{tarih}.json"
            st.download_button("⬇️  JSON Dışa Aktar",
                               data=json_bytes, file_name=json_ad,
                               mime="application/json",
                               use_container_width=True,
                               help="Random Forest modeli için ham veri")
...
if calistir and form_tamam and yuklenen_video:
    ...
    try:
        ...
        for idx_f, frame_no in enumerate(analiz_edilecek):
            ...
            overlay = overlay_ciz(frame.copy(), lms, skor, mode=annotation_mode, yuz_blur=yuz_blur)
    ...
    if not video_sonuclari:
        ...
        # Yüksek risk maruziyet (≥8)
        yuksek_risk_kare = sum(1 for s in tum_skorlar if s >= 8)
        yuksek_risk_sure = yuksek_risk_kare / 2  # 2fps → her kare 0.5sn
        yuksek_risk_pct = yuksek_risk_kare / len(tum_skorlar) * 100
        ...
        <div class="metric-row">
            ...
            <div class="metric-box">
                <div class="val" style="color:#dc2626">{yuksek_risk_sure:.0f}s</div>
                <div class="lbl">Yüksek Risk Maruziyeti</div>
                <div style="font-size:10px;color:#64748b;margin-top:3px">
                    {yuksek_risk_pct:.0f}% · {yuksek_risk_kare} kare
                </div>
            </div>
        </div>
        """, unsafe_allow_html=True)

        # Maruziyet özet mesajı
        if yuksek_risk_sure > 0:
            st.markdown(f"""
            <div class="warn-box">
            🔴 Çalışan <strong>{sure_sn:.0f} saniyenin {yuksek_risk_sure:.0f} saniyesini</strong>
            Yüksek Risk (REBA ≥8) postüründe geçirdi
            (%{yuksek_risk_pct:.0f} sürekli maruziyet).
            </div>
            """, unsafe_allow_html=True)
        ...
        with col_vb:
            vjson = video_analiz_export(
                form_bilgi_v, video_sonuclari, sure_sn,
                video_adi=yuklenen_video.name)
            vjson_ad = f"REBA_Video_{(bolum or is_istasyonu or 'analiz').replace(' ','_')}_{tarih}.json"
            st.download_button("⬇️  JSON Dışa Aktar",
                               data=vjson, file_name=vjson_ad,
                               mime="application/json",
                               use_container_width=True,
                               help="Random Forest modeli için ham veri")
