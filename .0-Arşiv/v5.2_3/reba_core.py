...
class AcilarObj:
    ...
    def __init__(self):
        ...
        self.segment_guven: dict = {}
        self.bacak_gozukuyor: bool = True
        self.kamera_acisi: str = "oblique"   # frontal / lateral / oblique
...
def vucut_acilari_hesapla(landmarks, w: int, h: int) -> AcilarObj:
    ...
    # ── KAMERA AÇISI SINIFLANDIRICI ──
    # omuz_gen / kalca_gen oranı:
    #   < 0.5  → lateral (yan çekim)   — üst kol ve bilek confidence düşürülür
    #   > 1.5  → frontal (ön çekim)    — gövde rotation görünmez
    #   arada  → oblique (çapraz çekim)
    omuz_gen = abs(p(LM.LEFT_SHOULDER)[0] - p(LM.RIGHT_SHOULDER)[0])
    kalca_gen = abs(p(LM.LEFT_HIP)[0] - p(LM.RIGHT_HIP)[0])
    if kalca_gen > 10:  # sıfır bölme koruması
        oran = omuz_gen / kalca_gen
        if oran < 0.5:
            a.kamera_acisi = "lateral"
        elif oran > 1.5:
            a.kamera_acisi = "frontal"
        else:
            a.kamera_acisi = "oblique"
    else:
        a.kamera_acisi = "oblique"

    # Lateral çekimde üst kol ve bilek güvenini %20 düşür
    if a.kamera_acisi == "lateral":
        a.segment_guven['ust_kol'] = max(0.0, a.segment_guven.get('ust_kol', 1.0) - 0.20)
        a.segment_guven['bilek']   = max(0.0, a.segment_guven.get('bilek', 1.0) - 0.20)
    ...
    # NOT: boyun_extension, boyun_yan_egim, boyun_donus → sidebar'dan manuel

    # ── GÖVDE — sadece fleksiyon açısı (modifier'lar manuel) ──
    a.govde_flexion = govde_aci_ham
    # NOT: govde_extension, govde_yan_egim, govde_donus → sidebar'dan manuel
